...
class GameState {
  ...
  async #loadData() {
    try {
      ...
      console.warn('Failed to load card data:', error.message);
      return false;
    }
  }
  ...
  #startNextRound() {
    ...
    this.cards = selected.map(mapCardData).filter(Boolean);
  ...
  endGame() {
    ...
    this.highestLevel = parseInt(localStorage.getItem(highScoreKey), 10) - 1;
    this.gameEnded = true;
  }
